...
2. **Hvis tool'et ikke er tilgængeligt** (første turn i session — MCP tools loader lazy), brug Bash-fallback:
   ```
   node /Users/karlomacmini/clawd/dearuser/mcp/run-tool.mjs history 2>/dev/null
   ```
   Med parametre (JSON):
   ```
   node /Users/karlomacmini/clawd/dearuser/mcp/run-tool.mjs history '{"format":"trend","scope":"security"}' 2>/dev/null
   ```
