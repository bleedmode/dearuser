...
function classifyRule(text, sectionContext) {
    ...
    if (PROHIBITION_PATTERNS.some(p => p.test(combined)))
        return 'prohibition';
    if (ASK_FIRST_PATTERNS.some(p => p.test(combined)))
        return 'ask_first';
    if (SUGGEST_PATTERNS.some(p => p.test(combined)))
        return 'suggest_only';
    if (AUTONOMOUS_PATTERNS.some(p => p.test(combined)))
        return 'do_autonomously';
    // Default based on section context
    if (/spørg|ask first/i.test(sectionContext))
        return 'ask_first';
    if (/foreslå|suggest/i.test(sectionContext))
        return 'suggest_only';
    if (/gør selv|do yourself|autonomous/i.test(sectionContext))
        return 'do_autonomously';
...
function countProjects(content) {
    ...
    if (tableMatches) {
        ...
        const dataRows = tableMatches.filter(row => !row.includes('---') && !row.includes('Projekt') && !row.includes('Project') && !row.includes('Name'));
