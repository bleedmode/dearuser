...
function scoreRoleClarity(parsed) {
    ...
    const hasScopeRules = parsed.rules.some(r => /beyond.?scope|only.?what|ændr.?ikke|ud.?over/i.test(r.text));
    const hasRoleDefinitions = parsed.rules.some(r => /\b(ceo|executor|agent|meta|owner|developer)\b/i.test(r.text));
    if (hasRolesSection)
        present.push('Roles section defined');
    else
        missing.push('No roles section');
    if (hasAutonomySection)
        present.push('Autonomy levels defined');
    else
        missing.push('No autonomy levels');
    if (hasScopeRules)
        present.push('Scope boundaries set');
    else
        missing.push('No scope boundaries');
    if (hasRoleDefinitions)
        present.push('Role definitions present');
    else
        missing.push('No explicit role definitions');
...
function scoreCommunication(parsed) {
    ...
    if (/\b(dansk|danish|english|spanish|language|sprog)\b/i.test(text))
        present.push('Language preference set');
    else
        missing.push('No language preference');
    if (/\b(short|concise|brief|kort|klart|terse)\b/i.test(text))
        present.push('Verbosity preference set');
    else
        missing.push('No verbosity preference');
    if (/\b(jargon|technical|analogi|business.?language|non.?technical)\b/i.test(text))
        present.push('Tone/style defined');
    else
        missing.push('No tone/style guidance');
    if (/\b(emoji|markdown|format|code.?block)\b/i.test(text))
        present.push('Format preferences set');
    else
        missing.push('No format preferences');
    const score = Math.round((present.length / (present.length + missing.length)) * 100);
...
function scoreAutonomyBalance(parsed) {
    ...
    if (doRules > 0)
        present.push(`${doRules} autonomous rules`);
    else
        missing.push('No autonomous action rules');
    if (askRules > 0)
        present.push(`${askRules} ask-first rules`);
    else
        missing.push('No ask-first rules');
    if (suggestRules > 0)
        present.push(`${suggestRules} suggest-only rules`);
    else
        missing.push('No suggest-only rules');
...
function scoreQualityStandards(parsed, scan) {
    const present = [];
    const missing = [];
    if (scan.hooksCount > 0)
        present.push(`${scan.hooksCount} hooks configured`);
    else
        missing.push('No hooks (automated quality gates)');
    const text = parsed.sections.map(s => s.content).join(' ') + parsed.rules.map(r => r.text).join(' ');
    if (/\b(test|jest|vitest|pytest|spec)\b/i.test(text))
        present.push('Testing mentioned');
    else
        missing.push('No testing strategy');
    if (/\b(build|compile|tsc|eslint|lint)\b/i.test(text))
        present.push('Build/lint verification');
    else
        missing.push('No build verification');
    if (/\b(done|kvalitet|quality|definition.?of.?done|complete)\b/i.test(text))
        present.push('Definition of done');
    else
        missing.push('No definition of done');
    const score = Math.round((present.length / (present.length + missing.length)) * 100);
...
function scoreMemoryHealth(parsed, scan) {
    ...
    if (memCount > 0)
        present.push(`${memCount} memory files`);
    else
        missing.push('No memory files');
    if (feedbackCount > 0)
        present.push(`${feedbackCount} feedback memories (learning loop)`);
    else
        missing.push('No feedback memories');
    if (parsed.learnings.length > 0)
        present.push(`${parsed.learnings.length} learnings documented`);
    else
        missing.push('No learnings documented');
    ...
    if (recentMemories.length > 0)
        present.push(`${recentMemories.length} memories updated in last 30 days`);
    else if (memCount > 0)
        missing.push('No recently updated memories');
    const score = Math.round((present.length / (present.length + missing.length)) * 100);
...
function scoreSystemMaturity(scan) {
    ...
    for (const a of artifacts) {
        if (a.count > 0)
            present.push(`${a.count} ${a.name.toLowerCase()}`);
        else
            missing.push(`No ${a.name.toLowerCase()}`);
    }
    // Score on a curve
    ...
    if (totalArtifacts === 0)
        score = 10;
    else if (totalArtifacts <= 2)
        score = 30;
    else if (totalArtifacts <= 5)
        score = 50;
    else if (totalArtifacts <= 10)
        score = 70;
    else if (totalArtifacts <= 15)
        score = 85;
    else
        score = 95;
...
export function score(parsed, scan) {
    const categories = {
    ...
    // Weighted aggregate
    const collaborationScore = Math.round(Object.values(categories).reduce((sum, cat) => sum + cat.score * cat.weight, 0));
